...
class CalculatorSkill(BaseSkill):
    ...
    async def execute(self, expression: str) -> Dict[str, Any]:
        ...
        dangerous_patterns = [
            ...
            r'globals\s*\(',  # globals 函数
            r'locals\s*\(',  # locals 函数
            r'getattr\s*\(',  # getattr 函数
            r'setattr\s*\(',  # setattr 函数
            r'delattr\s*\(',  # delattr 函数
            r'hasattr\s*\(',  # hasattr 函数
        ]

        for pattern in dangerous_patterns:
            if re.search(pattern, expression, re.IGNORECASE):
                logger.warning(
                    "Dangerous pattern detected in calculator expression: %s",
                    pattern
                )
                return {
                    "expression": expression[:50] + "...",
        ...
        try:
            ...
            evaluator.functions.update(safe_math_functions)
            ...
            logger.debug("Calculator error: %s", str(e))
            return {
                "expression": expression,
                "error": str(e),
                "success": False
...
class SkillsRegistry:
    ...
    async def execute_skill(
        ...
        Raises:
            ValueError: 技能不存在或参数验证失败
        """
        skill = self.get_skill(name)
        if not skill:
            raise ValueError(f"技能不存在: {name}")

        # 参数验证（如果 jsonschema 可用）
        if HAS_JSONSCHEMA and skill.parameters:
            try:
                validate(instance=arguments, schema=skill.parameters)
            except SchemaValidationError as e:
                logger.warning(
                    "Skill parameter validation failed: name=%s, error=%s",
                    name,
                    e.message
                )
