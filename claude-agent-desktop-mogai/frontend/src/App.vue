  ...
  <div class="app-container">
    ...
    <header class="app-header">
      ...
      <div class="app-actions">
        ...
        >
          <el-icon>
            <Sunny v-if="getThemeIcon() === 'Sun'" />
            <Moon v-else-if="getThemeIcon() === 'Moon'" />
            <Monitor v-else />
          </el-icon>
        </el-button>
        <el-button
    ...
    <!-- Element Plus 全局组件由 useToast/useConfirm 自动管理，无需手动挂载 -->
  </div>
</template>
...
// Element Plus 图标
import { Plus, Sunny, Moon, Monitor, Fold, Expand, QuestionFilled } from '@element-plus/icons-vue'
import ChatInterface from './components/ChatInterface.vue'
import SessionList from './components/SessionList.vue'
import SkillPanel from './components/SkillPanel.vue'
// ToastContainer 和 GlobalDialog 已迁移到 Element Plus
// useToast 现在直接使用 ElMessage，useConfirm 使用 ElMessageBox
...
const { cycleTheme, getThemeIcon, getThemeName } = useTheme()
  ...
  try {
    ...
    await chatApi.sendMessageStream(
      ...
      (err: Error) => {
        ...
        messagesMap.value.set(currentSessionId.value, [...messages])
        isLoading.value = false
        error(err.message || '网络错误，请检查网络连接')
      }
    )
  } catch (err) {
